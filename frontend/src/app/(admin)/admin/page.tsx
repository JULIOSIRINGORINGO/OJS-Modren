...
import { FileText, Clock, CheckCircle, XCircle, Users, Settings, Library, Tag, Loader2 } from "lucide-react";
import { StatsCard } from "@/components/dashboard/StatsCard";
import { fetchArticles, fetchUsers } from "@/lib/api-client";
...
export default function AdminOverviewPage() {
  ...
  const reviewerCount = users.filter((u) => u.role?.toLowerCase() === "reviewer").length;
        ...
        <div className="px-6 py-8 max-w-6xl w-full mx-auto">
          {loading ? (
            <div className="flex items-center justify-center py-24">
              ...
              <span className="ml-3 text-sm font-bold text-muted-foreground">Memuat data admin...</span>
            </div>
          ) : (
            <>
              {/* Stats */}
              ...
              >
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                  {[
                    { label: "Total Pengguna", value: users.length.toString(), icon: Users, bg: "bg-indigo-100 text-indigo-700" },
                    { label: "Penulis", value: authorCount.toString(), icon: FileText, bg: "bg-blue-100 text-blue-700" },
                    { label: "Peninjau", value: reviewerCount.toString(), icon: CheckCircle, bg: "bg-emerald-100 text-emerald-700" },
                    { label: "Editor", value: editorCount.toString(), icon: Settings, bg: "bg-amber-100 text-amber-700" },
                  ].map((stat) => (
